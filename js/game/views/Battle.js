...
Battle.prototype.onOpen = function (params, cb) {
	...
	server.send('demo.' + params.battleType, params, function (error, data) {
		...
		// preload images
		var imgList = [];
		imgList.push(self.bk);
		imgList.push(self.grid);
		for (var i in self.img) {
			imgList.push(self.img[i]);
		}
		for (var i = 0; i < result.myTeam.length; i++) { 
			imgList.push(result.myTeam[i].sprite);
		}
		for (var i = 0; i < result.opponentTeam.length; i++) { 
			imgList.push(result.opponentTeam[i].sprite);
		}
		var loader = new Loader();
		loader.asyncImg(imgList, function () {
...
Battle.prototype.createTurn = function (fightLog, counter, mf, of) {
	...
	var elm = fightLog[counter].action.element;	
	var style = 'background: url(' + this.img[elm] + ') no-repeat; width: 20px; height: 25px; line-height: 25px; -webkit-background-size: 100%;';
	...
	a.onStart = function () {
		...
		if (elm === 'phys') {
			audio = 'punch';
		} else if (elm === 'almighty') {
			audio = 'laser';
		}
		window.game.playAudio(audio);
		// show skill name now
