...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : window.assets['nicEditor'],
...
var nicEditor = bkClass.extend({
	...
	instance: function (index) {
		if (this.nicInstances[index]) {
			return this.nicInstances[index];
		}
		return null;
	},
...
var nicEditorInstance = bkClass.extend({
		...
		if(r && r.startContainer) {
			...
			if(r.cloneContents().childNodes.length == 1) {
				for(var i=0;i<contain.childNodes.length;i++) {
					if (contain.childNodes[i].ownerDocument) {
	...
	setImagePath : function (path) {
		window.__nicEditor_inject_image_path = path;
	}		
});
...
var nicEditorButton = bkClass.extend({
	...
	checkNodes : function(e) {
		...
		while(elm = elm.parentNode && elm.className != "nicEdit") {
			if(elm && this.options.tags && bkLib.inArray(this.options.tags,elm.nodeName)) {
				this.activate();
				return true;
			}
		}
...
var nicLinkButton = nicEditorAdvancedButton.extend({	
	addPane : function() {
		if (this.ne.selectedInstance.selElm()) {
...
var nicEditorColorButton = nicEditorAdvancedButton.extend({	
	addPane : function() {
			var colorList = ['00', '33', '66', '99', 'AA', 'CC', 'DD', 'EE', 'FF'];
			...
			for(var r in colorList) {
				for(var b in colorList) {
					for(var g in colorList) {
						...
						var colorSquare = new bkElement('DIV').setStyle({'cursor' : 'pointer', 'height' : (size+(border * 2))+'px', 'float' : 'left'}).appendTo(colorItems);
						var colorBorder = new bkElement('DIV').setStyle({border: border+'px solid '+colorCode}).appendTo(colorSquare);
						colorBorder.setAttribute('title', colorCode);
						var colorInner = new bkElement('DIV').setStyle({backgroundColor : colorCode, overflow : 'hidden', width : size+'px', height : size+'px'}).addEvent('click',this.colorSelect.closure(this,colorCode)).addEvent('mouseover',this.on.closure(this,colorBorder)).addEvent('mouseout',this.off.closure(this,colorBorder,colorCode)).appendTo(colorBorder);
						if(!window.opera) {
	...
	on : function(colorBorder) {
		colorBorder.setStyle({border : '2px solid #FFFFFF'});
	},
...
var nicImageButton = nicEditorAdvancedButton.extend({	
	addPane : function() {
		var path = false;
		var img = '';
		if (window.__nicEditor_inject_image_path){
			path = window.__nicEditor_inject_image_path;
			img = '&nbsp;&nbsp;<img src="'+path+'" width="30" style="border: 1px solid #999999; padding: 1px;" />';
		...
		this.addForm({
			'' : {type : 'title', txt : 'Add/Edit Image'+img},
			'src' : {type : 'text', txt : 'URL', 'value' : path, style : {width: '150px'}},
...
var nicUploadButton = nicEditorAdvancedButton.extend({	
	nicURI : '/ajax/handle_cimg_upload.php',
	...
	addForm : function() {
		...
		myDoc.write('<input name="cimg" type="file" style="margin-top: 10px;" />');
		myDoc.write('</form>');
	...
	startUpload : function() {
		...
		this.myStatus.setContent('<img src="' + window.spinners['preloader'] + '" style="float: right; margin-right: 40px;" /><strong>Uploading...</strong><br />Please wait');
		this.myForm.submit();
