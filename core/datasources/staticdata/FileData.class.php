...
class FileData {
	...
	* }
	*/
	public function FileData($confName) {
		$this->cache = new Cache();
		$conf = Config::get($confName);
		if (!$conf) {
			Log::error('[FILEDATA] constructor: missing configurations');
		}
		if (!isset($conf['sourcePath'])) {
			Log::error('[FILEDATA] constructor: missing configuration "sourcePath"');
		} else {
			$this->srcPath = $conf['sourcePath'];
		}
		if (!isset($conf['csvParseRules'])) {
			Log::error('[FILEDATA] constructor: missing configuration "csvRules"');
		} else {
	...
	private function getData($fileNames) {
		...
		try {
			...
			Log::verbose('[FILEDATA] getData: ' . implode(',', $fileNames) . ' took [' . $time. ' msec] to execute');
			return $mergedData;
		} catch (Exception $e) {
			Log::error('[FILEDATA] getData: ' . $e->getMessage());
			return null;
		}
	}
